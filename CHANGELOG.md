...
- Active-plugin update orchestration (Issue #22 PR3): when mutation is enabled (default), `numan update` runs deactivate→upgrade→reactivate via `cmd::plugin_lifecycle`; kill switch `NUMAN_ENABLE_ACTIVE_PLUGIN_MUTATION=0`
...
- Active-plugin **update** is permitted by default (orchestrated); remove remains gated until deactivate
